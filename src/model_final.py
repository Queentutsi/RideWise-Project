"""
RideWise Churn Classification Pipeline final
Author: Oluwatosin
"""
...
# Models (all available without extra installation)
...
def get_models():
    return {
        ...
        "Gradient Boosting": GradientBoostingClassifier(
            random_state=42
            # NOTE: GradientBoosting does NOT support class_weight
        ),
        ...
        "KNN": KNeighborsClassifier(
            n_neighbors=7
            # NOTE: KNN does NOT support class_weight
        ),
        ...
        "XGBoost": XGBClassifier(
            ...
            random_state=42,
            scale_pos_weight=2  # class weighting for XGBoost
        ),

        "AdaBoost": AdaBoostClassifier(
            n_estimators=300,
            random_state=42
            # NOTE: AdaBoost does NOT support class_weight directly
        ),

        "Bagging": BaggingClassifier(
            n_estimators=300,
            random_state=42
            # NOTE: Bagging does NOT support class_weight directly
        )
    }
...
def plot_confusion_matrix(cm, model_name):
    ...
    ax.set_title(f"{model_name} 3 – Confusion Matrix")
    ax.set_xlabel("Predicted")
    ax.set_ylabel("Actual")
    save_plot(fig, f"{model_name}_3_confusion_matrix.png")
...
def plot_roc_curve(y_test, y_prob, model_name):
    ...
    ax.plot([0, 1], [0, 1], "k--")
    ax.set_title(f"{model_name} 3 – ROC Curve")
    ax.set_xlabel("False Positive Rate")
    ax.set_ylabel("True Positive Rate")
    ax.legend()
    save_plot(fig, f"{model_name}_3_ROC_curve.png")
...
def plot_precision_recall(y_test, y_prob, model_name):
    ...
    ax.set_title(f"{model_name} 3 – Precision–Recall Curve")
    ax.set_xlabel("Recall")
    ax.set_ylabel("Precision")
    save_plot(fig, f"{model_name}_3_PR_curve.png")
...
def plot_model_comparison(results_df):
    ...
    for metric in metrics:
        ...
        sns.barplot(data=results_df, x="Model", y=metric, hue="Model", legend=False, palette="viridis", ax=ax)
        ax.set_xticklabels(results_df["Model"], rotation=90)
        ax.set_title(f"Model Comparison – {metric}")
        save_plot(fig, f"model_comparison3_{metric}.png")
...
def main():
    ...
    # Select best model
    best_model_name3 = results.sort_values("AUC", ascending=False).iloc[0]["Model"]
    best_model = models[best_model_name3]

    print(f"\nBest Model: {best_model_name3}")

    # Predictions
    if best_model_name3 in ["Logistic Regression", "KNN", "SVC (RBF)"]:
    ...
    plot_confusion_matrix(cm, best_model_name3)
    plot_roc_curve(y_test, y_prob, best_model_name3)
    plot_precision_recall(y_test, y_prob, best_model_name3)
