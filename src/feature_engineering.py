...
def build_features(input_path: str) -> pd.DataFrame:
    ...
    # Fill any out-of-range or missing values with the "young" bucket.
    df["age_bucket"] = df["age_bucket"].fillna("young")

    # Convert the bucket labels to numeric codes for model training.
    age_bucket_map = {"young": 0, "early_adult": 1, "mid_adult": 2, "senior": 3}
    df["age_bucket"] = df["age_bucket"].map(age_bucket_map)
    ...
    if "churn_probability" in df.columns:
        ...
        raise KeyError("No churn target column found in the input data.")
    ...
    if target_col == "churn_prob":
        df_features = df_features.rename(columns={"churn_prob": "churn_probability"})
